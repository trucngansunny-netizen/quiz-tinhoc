...
def grade_ppt(file_path, criteria):
    ...
    def slide_has_picture(slide):
        for shape in slide.shapes:
            try:
                ...
                if any(tag in xml_str for tag in ["p:pic", "a:blip", "a:blipfill", "blipfill", "img", ".jpg", ".png", ".gif", ".jpeg", ".webp", ".svg", ".bmp", ".tiff", ".ico", ".wmf", ".emf"]):
                    return True
                if shape.shape_type == MSO_SHAPE_TYPE.GROUP and any(
                    slide_has_picture(s) for s in shape.shapes
                ):
                    return True
            except Exception:
                continue
    ...
    # Loại trừ slide trống hoặc chỉ có text
    has_picture_any = False
    for slide in slides:
        if slide_has_picture(slide):
            shapes_text = " ".join(shape.text for shape in slide.shapes if getattr(shape, "has_text_frame", False))
            if len(shapes_text.strip()) < 10:  # ít chữ, có khả năng là slide chứa hình
...
def analyze_sb3_basic(file_path):
    ...
    try:
        ...
        flags = {
            "has_loop": False, "has_condition": False, "has_interaction": False, "has_variable": False, "multiple_sprites_or_animation": False
        }
